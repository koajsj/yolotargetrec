...
- **摄像头抽帧间隔**:`250 ms`(见 `frontend/app.js` 的 `CAMERA_INTERVAL_MS`)
- **抽帧最大宽度**:`640 px`,按比例缩放以降低带宽
- **跟踪策略**:`backend/tracker.py` 实现轻量 IoU 匹配;若跟踪失败,后端在响应中回退为单帧递增编号
